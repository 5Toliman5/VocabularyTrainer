...
namespace VocabularyTrainer.WinApp
{
	public partial class MainForm
	{
		...
		public void SetCurrentWordDictionary(string dictionaryName)
		{
			DictionaryOfWordLabel.Text = dictionaryName;
		}

		public void SetShowNextButtonText(string text)
		{
		...
		private int? SelectedTrainingDictionaryId
		{
			get
			{
				return DictionaryTrainingComboBox.SelectedItem is DictionaryComboItem item
					? item.Id
					: null;
			}
		}

		private void PopulateTrainingComboBox(IReadOnlyList<DictionaryDto> dictionaries)
		{
			...
			{
				DictionaryTrainingComboBox.Items.Add(new DictionaryComboItem(dictionary.Id, dictionary.ToString()));
			}
		...
		private void ShowNextWord(object sender, EventArgs e)
		{
			ShowNextWordRequested?.Invoke(sender, e);
		}

		private void ShowTranslation(object sender, EventArgs e)
		{
			ShowTranslationRequested?.Invoke(sender, e);
		}
