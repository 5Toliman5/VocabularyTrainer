...
namespace VocabularyTrainer.WinApp
{
	public partial class MainForm
	{
		...
		public int? MyWordsDictionaryId
		{
			get
			{
				return MyWordsDictionaryCombo.SelectedItem is DictionaryComboItem item && item.Id.HasValue
					? item.Id
					: null;
			}
		}
		...
		public string MyWordsLanguage
		{
			get
			{
				return MyWordsLanguageCombo.SelectedItem is string code && code != string.Empty
					? code
					: null;
			}
		}
		...
		public string MyWordsSearch
		{
			get
			{
				return string.IsNullOrWhiteSpace(MyWordsSearchBox.Text)
					? null
					: MyWordsSearchBox.Text.Trim();
			}
		}
		...
		public DateTime? MyWordsDateFrom
		{
			get
			{
				return MyWordsDateFromPicker.Checked
					? MyWordsDateFromPicker.Value.Date
					: null;
			}
		}
		...
		public DateTime? MyWordsDateTo
		{
			get
			{
				return MyWordsDateToPicker.Checked
					? MyWordsDateToPicker.Value.Date.AddDays(1).AddTicks(-1)
					: null;
			}
		}
		...
		public WordDto? SelectedMyWordsWord
		{
			get
			{
				if (MyWordsGrid.SelectedRows.Count == 0)
				{
					return null;
				}
				...
				if (index < 0 || index >= _currentMyWordsWords.Count)
				{
					return null;
				}
		...
		private void InitializeMyWordsGrid()
		{
			...
			MyWordsGrid.Columns.AddRange(
				...
				new DataGridViewTextBoxColumn { Name = "Translation", DataPropertyName = "PrimaryTranslation", HeaderText = "Translation", Width = 252, SortMode = DataGridViewColumnSortMode.Programmatic },
		...
		private void PopulateMyWordsDictionaryFilter(IReadOnlyList<DictionaryDto> dictionaries)
		{
			...
			{
				MyWordsDictionaryCombo.Items.Add(new DictionaryComboItem(dictionary.Id, dictionary.ToString()));
			}
		...
		private void PopulateMyWordsLanguageFilter(IReadOnlyList<DictionaryDto> dictionaries)
		{
			...
			var languageCodes = dictionaries
				.Where(d => !string.IsNullOrEmpty(d.LanguageCode))
