...
namespace VocabularyTrainer.DataAccess.SqlQueries
{
	public class WordSqlQueries
	{
		...
		public const string InsertWord =
			$"INSERT INTO Words(Value, Translation) OUTPUT INSERTED.ID VALUES(@{nameof(Word.Value)},@{nameof(Word.Translation)})";

		public const string DeleteWord =
			$"DELETE FROM Words WHERE Id = @{nameof(EditWordRequest.WordId)}";
		...
		public const string InsertUserWord =
			...
			$"VALUES(@{nameof(EditWordRequest.WordId)}, @{nameof(EditWordRequest.UserId)}, @{nameof(EditWordRequest.DictionaryId)}, 0)";

		private const string UserWordsWhereClause =
			$" WHERE WordId = @{nameof(EditWordRequest.WordId)} AND UserId = @{nameof(EditWordRequest.UserId)}";

		private const string UserWordsDictionaryWhereClause =
			UserWordsWhereClause + $" AND DictionaryId = @{nameof(EditWordRequest.DictionaryId)}";
		#endregion
	}
}
